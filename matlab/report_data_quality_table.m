...
txt = sprintf('For %d participants, the average inaccuracy in the data determined from a %d-point validation procedure using ETDQualitizer v%s (Niehorster et al., in prep) was %.2f° (SD=%.2f°, range=%.2f°--%.2f°). Average RMS-S2S precision was %.3f° (SD=%.3f°, range=%.3f°--%.3f°) and STD precision %.3f° (SD=%.3f°, range=%.3f°--%.3f°).',n_subj,n_target,version,measures.mean.accuracy,measures.std.accuracy,measures.min.accuracy,measures.max.accuracy,measures.mean.rms_s2s,measures.std.rms_s2s,measures.min.rms_s2s,measures.max.rms_s2s,measures.mean.std,measures.std.std,measures.min.std,measures.max.std);
