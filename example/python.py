...
for f in (pathlib.Path(__file__).parent / 'data').glob('*.tsv'):
    ...
    for e in ('left','right'):
        ...
        print(f'Data loss from invalid samples ({e} eye): {dq.data_loss_from_invalid():.1f}%')
        print(f'Data loss from expected #samples ({e} eye): {dq.data_loss_from_expected(fs):.1f}%')
